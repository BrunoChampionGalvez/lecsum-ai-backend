...
export class QuizzesController {
  @Post()
  async createQuiz(
    @Body() createDto: { title: string; courseId: string; questions: { question: string; correctAnswer: string; options: string[] }[] },
    @Request() req,
  ): Promise<Quiz> {
  ...
  async updateQuiz(
    @Param('id') id: string,
    @Body() updateDto: { title: string; questions: { question: string; correctAnswer: string; options: string[] }[] },
    @Request() req,
  ): Promise<Quiz> {
  ...
  @Get('course/:courseId')
  async findAllByCourse(@Param('courseId') courseId: string, @Request() req): Promise<Quiz[]> {
    return this.quizzesService.findAllByCourse(courseId, req.user.id);
  }

  @Get(':id')
  async findOne(@Param('id') id: string, @Request() req): Promise<Quiz> {
  ...
  @Get(':id/details')
  async findQuizDetails(@Param('id') id: string, @Request() req): Promise<any> {
    return this.quizzesService.findQuizDetails(id, req.user.id);
  }

  @Get('question/:id')
  async findQuestionDetails(@Param('id') id: string, @Request() req): Promise<any> {
    return this.quizzesService.findQuestionDetails(id, req.user.id);
  }

  @Post('questions/batch')
  async findQuestionsDetailsBatch(@Body() body: { ids: string[] }): Promise<Record<string, any>> {
