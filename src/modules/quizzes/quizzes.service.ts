...
export class QuizzesService {
  async createQuiz(
    userId: string,
    createDto: { title: string; courseId: string; questions: { question: string; correctAnswer: string; options: string[] }[] }
  ): Promise<Quiz> {
    ...
    for (const q of createDto.questions) {
  ...
  async updateQuiz(
    id: string,
    userId: string,
    updateDto: { title: string; questions: { question: string; correctAnswer: string; options: string[] }[] }
  ): Promise<Quiz> {
  ...
  async generateQuiz(
    ...
    const questionData = parsedQuestions.map((q: any) => ({
      question: q.question,
      correctAnswer: q.correctAnswer,
      options: q.options,
      aiGenerated: true,
  ...
  async submitQuizAnswers(
    ...
    for (const answer of answers) {
      ...
      if (!question) {
        throw new BadRequestException(`Question with ID ${answer.questionId} not found in this quiz`);
      }
  ...
  async findQuizDetails(id: string, userId: string): Promise<any> {
    const quiz = await this.quizzesRepository.findOne({
      where: { id },
  ...
  async getQuestionPath(questionId: string, userId: string): Promise<string> {
    ...
    if (!quiz) {
      throw new NotFoundException(`Quiz for question with ID ${questionId} not found`);
    }
    ...
    const position = quiz.questions.findIndex(q => q.id === questionId) + 1;
    
    return `${quiz.course.name}/${quiz.title}/Question ${position}`;
  }

  async findQuestionDetails(id: string, userId: string): Promise<any> {
    const question = await this.questionsRepository.findOne({
      where: { id },
  ...
  async findQuestionsDetailsBatch(ids: string[]): Promise<Record<string, any>> {
    if (!ids || ids.length === 0) {
      return {};
    }
