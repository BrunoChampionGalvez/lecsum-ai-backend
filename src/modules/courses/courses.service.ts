...
export class CoursesService {
  ...
  async findAllContentOfOne(id: string, userId: string): Promise<{id: string, name: string, content: string, type: string, originalName: string}[]> {
    ...
    for (const folder of course.folders) {
      const folderFiles = await this.foldersService.findAllFilesRecursively(folder.id, userId);
  ...
  async create(courseData: { name: string; description?: string }, userId: string): Promise<Course> {
  ...
  async update(id: string, courseData: { name?: string; description?: string }, userId: string): Promise<Course> {
    const course = await this.findOne(id, userId);
