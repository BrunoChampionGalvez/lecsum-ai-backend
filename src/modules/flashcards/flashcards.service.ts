...
export class FlashcardsService {
  ...
  async findAllByCourse(courseId: string, userId: string): Promise<Flashcard[]> {
  ...
  async generateFlashcards(
    ...
    let fileContents: Array<{ id: string; name: string; content: string; type: string }> = [];
    let newFileContents: Array<{ id: string; name: string; content: string; type: string }> = [];
    // Get files from folders if folderIds are provided
    if (params.folderIds && params.folderIds.length > 0) {
      const files = await Promise.all(
        params.folderIds.map(folderId => this.foldersService.findAllFilesRecursively(folderId, userId))
      );
    ...
    fileContents = fileContents.concat(newFileContents).filter((value, index, self) => {
      const isDuplicate = self.findIndex(item => item.id === value.id) !== index;
      return !isDuplicate;
    });
    
    // Only require file selection if no folders were selected
    if ((!params.fileIds || params.fileIds.length === 0) && fileContents.length === 0) {
      throw new BadRequestException('At least one file or folder must be selected');
    }
    
    if (!params.types || params.types.length === 0) {
      throw new BadRequestException('At least one flashcard type must be selected');
    }
    ...
    const directlySelectedFiles = params.fileIds && params.fileIds.length > 0 ?
      await Promise.all(params.fileIds.map(fileId => this.filesService.findOne(fileId, userId))) :
      [];
      
    // Combine directly selected files with files from folders
    const files = [...directlySelectedFiles, ...fileContents].filter((value, index, self) => {
      const isDuplicate = self.findIndex(item => item.id === value.id) !== index;
      return !isDuplicate;
    });
    ...
    try {
      parsedFlashcards = JSON.parse(generatedFlashcards
        .replace(/^\s*```[a-z]*\s*/i, '')
        .replace(/\s*```[\s\n]*$/, '')
        .trim());
    ...
    const flashcardData = parsedFlashcards.map((fc: any) => ({
      type: fc.type,
      front: fc.front,
      back: fc.back,
    ...
    const savedFlashcards = await this.flashcardsRepository.save(
      flashcardData as Partial<Flashcard>[]
    );
  ...
  async findDeckById(id: string, userId: string): Promise<any> {
  ...
  async getFlashcardPath(flashcardId: string, userId: string): Promise<string> {
    ...
    if (!deck) {
      throw new NotFoundException(`Deck for flashcard with ID ${flashcardId} not found`);
    }
    
    // Find the position of this flashcard in the deck
    const position = deck.flashcards.findIndex(f => f.id === flashcardId) + 1;
    
    // Get course name
    const course = await this.coursesService.findOne(flashcard.courseId, userId);
  ...
  async create(data: Partial<Flashcard>, userId: string): Promise<Flashcard> {
  ...
  async update(id: string, data: Partial<Flashcard>, userId: string): Promise<Flashcard> {
    const flashcard = await this.flashcardsRepository.findOne({ where: { id } });
    if (!flashcard) throw new NotFoundException(`Flashcard with ID ${id} not found`);
    // Optionally verify deck/course ownership by userId here
