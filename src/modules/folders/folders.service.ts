...
export class FoldersService {
  ...
  async findFolderContents(folderId: string, userId: string): Promise<Folder[]> {
  ...
  async createFolder(courseId: string, userId: string, createFolderDto: CreateFolderDto): Promise<Folder> {
    ...
    if (createFolderDto.parentId) {
      ...
      if (parentFolder.courseId !== courseId) {
        throw new BadRequestException('Parent folder does not belong to this course');
      }
    }
   ...
   */
  private async deleteFolderContentsRecursively(folderId: string, userId: string): Promise<void> {
    ...
    if (folderWithFiles && folderWithFiles.files && folderWithFiles.files.length > 0) {
      ...
      await Promise.all(folderWithFiles.files.map(file => 
        this.filesRepository.remove(file)
      ));
    }
  }

  async moveFolder(id: string, userId: string, newParentId: string | null): Promise<Folder> {
    ...
    if (newParentId) {
      ...
      if (parentFolder.courseId !== folder.courseId) {
        throw new BadRequestException('Target folder does not belong to the same course');
      }
   ...
   */
  async findAllFilesRecursively(folderId: string, userId: string): Promise<File[]> {
    // Verify folder exists and belongs to user
    const folder = await this.findOne(folderId, userId);
    ...
    for (const subfolder of subfolders) {
      const subfolderFiles = await this.findAllFilesRecursively(subfolder.id, userId);
